...
if (!file.exists("data/steps/quotes_clean.feather")) {
  window <- 10
...
$$ P_{i, vol10} = (P_i - 100) / vol10_i + 100 $$
...
$$ P_{i, vol} = (P_i - 100) / median(vol10_i, j) + 100 $$
...
if (!file.exists(path_data_wide_10_vol)) {
  ...
  data_wide_10_sorted <- data_wide_10 %>% mutate(ID = seq_len(nrow(.))) %>% arrange(Ticker, Date)
  vol_10 <- data_wide_10_sorted %>%
  ...
  data_wide_10_vol <- data_wide_10_sorted %>% 
