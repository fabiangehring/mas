...
model_ind_low_path  <- "data/models/ind/categorical/2_dense_512/model_low.rds"
pred_ind_low_path  <- "data/models/ind/categorical/2_dense_512/pred_low.rds"
if (file.exists(model_ind_low_path) && file.exists(pred_ind_low_path)) {
  model_cat_low <- readRDS(model_ind_low_path)
  pred_cat_lowh <- readRDS(pred_ind_low_path)
} else {
  ...
  model_cat_low <- fit_model_cat(x[train_idx, ], y_low[train_idx], 10, callbacks = list(cp_callback_low))
  saveRDS(model_cat_low, model_ind_low_path)
  pred_cat_low <- predict_proba(model_cat_low$model, x[test_idx, ], batch_size = 512)
  saveRDS(pred_cat_low, pred_ind_low_path)
}

model_ind_high_path  <- "data/models/ind/categorical/2_dense_512/model_high.rds"
pred_ind_high_path  <- "data/models/ind/categorical/2_dense_512/pred_high.rds"
if (file.exists(model_ind_high_path) && file.exists(pred_ind_high_path)) {
  model_cat_high <- readRDS(model_ind_high_path)
  ...
  model_cat_high <- fit_model_cat(x[train_idx, ], y_high[train_idx], 10, callbacks = list(cp_callback_high))
  saveRDS(model_cat_high, model_ind_high_path)
  pred_cat_high <- predict_proba(model_cat_high$model, x[test_idx, ], batch_size = 512)
  saveRDS(pred_cat_high, pred_ind_high_path)
}

model_ind_close_path  <- "data/models/ind/categorical/2_dense_512/model_close.rds"
pred_ind_close_path  <- "data/models/ind/categorical/2_dense_512/pred_close.rds"
if (file.exists(model_ind_close_path)) {
  model_cat_close <- readRDS(model_ind_close_path)
  pred_cat_lclose<- readRDS(pred_ind_close_path)
} else {
  ...
  model_cat_close <- fit_model_cat(x[train_idx, ], y_close[train_idx], 10, callbacks = list(cp_callback))
  saveRDS(model_cat_close, model_ind_close_path)
...
train_history <- as_tibble(model_cat_low$history) %>% mutate(price = "low") %>%
  bind_rows(as_tibble(model_cat_high$history) %>% mutate(price = "high")) %>%
  bind_rows(as_tibble(model_cat_close$history) %>% mutate(price = "close")) %>%
  mutate(
