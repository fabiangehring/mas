...
if (file.exists(model_ind_bin_high_path) && file.exists(history_ind_bin_high_path)) {
  ...
  model_cat_bin_high <- fit_model_bin(x[train_idx, ], y_high[train_idx], n_classes = 30, callbacks = list(cp_callback_bin_high))
...
if (file.exists(model_ind_bin_close_path) && file.exists(history_ind_bin_close_path)) {
  ...
  model_cat_bin_close <- fit_model_bin(x[train_idx, ], y_close[train_idx], n_classes = 30, callbacks = list(cp_callback_bin_close))
...
plot_neural_sample_histogram(52, ind_binary)
...
if (file.exists(model_dep_path) && file.exists(history_dep_path)) {
  ...
  cp_callback_dep <- callback_model_checkpoint(
    filepath = file.path("data/models/dep/categorical/2_dense_512", "low.{epoch:02d}-{val_loss:.2f}.hdf5")
  )
...
dep_buy_sell_max_prob_path <- "data/models/dep/categorical/2_dense_512/ind_buy_sell_max_prob.rds"
if (file.exists(dep_buy_sell_max_prob_path)) {
  dep_buy_sell_max_prob <- read_feather(dep_buy_sell_max_prob_path)
} else {
  n_eval <- length(test_idx)
  
  probs <- predict_proba(model_dep, x[test_idx[seq_len(n_eval)], ], batch_size = 512)

  ind_buy_sell_max_prob <- lapply(seq_len(n_eval), function(i) {
    threshold <- quantile(probs[i, ], q)
    calc_best_buy_sell(probs[i, ], max(probs[i, ]), mid_dep_low, mid_dep_high, mid_dep_close, both_first = both_first[test_idx[i]])
  }) %>% bind_rows()
  write_feather(ind_buy_sell_max_prob, ind_buy_sell_max_prob_path)
}
