...
model_low_100 <- fit_categorical_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "Low_0", 100)
save_model_hdf5(model_low_100, "data/model_low_100.hdf5")

model_high_100 <- fit_categorical_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "High_0", 100)
save_model_hdf5(model_high_100, "data/model_high_100.hdf5")

model_close_100 <- fit_categorical_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "Close_0", 100)
save_model_hdf5(model_close_100, "data/model_close_100.hdf5")
...
get_class_prices <- function(borders, groups) {
  ...
  class_centers <- (class_borders$Lower_Border + class_borders$Upper_Border) / 2
  class_centers[groups + 1]
}

n_groups_per_col <- 100

# buy prices
model_low <- load_model_hdf5("data/model_low.hdf5")
pred_classes_low <- predict_classes(model_low, test_data, batch_size = 1024)
# pred_prob <- predict_proba(model_low, test_data, batch_size = 1024)

discretization_low <- multivariate_discretization(data_wide_3_all, train_idx, test_idx, "Low_0", n_groups_per_col)
buy <- get_class_prices(discretization_low$borders, pred_classes_low)


# sell prices
model_high <- load_model_hdf5("data/model_high_100.hdf5")
pred_classes_high <- predict_classes(model_high, test_data, batch_size = 1024)
# pred_prob <- predict_proba(model_high, test_data, batch_size = 1024)
...
unexpected_order_idx <- which(buy > sell)
buy[unexpected_order_idx] <- sell[unexpected_order_idx] <- (buy[unexpected_order_idx] + sell[unexpected_order_idx]) / 2


eval_data <- data_wide_3_all[test_idx, ] %>% select(Close_1, Open = Open_0, Low = Low_0, High = High_0, Close_0)

set.seed(654951)
both_first <- c("buy", "sell")[sample(c(1, 2), nrow(eval_data), replace = TRUE)]
...
# open
sum(calc_payoff_const_gamma(eval_data, buy = eval_data$Open * (1 - 0.05), sell = eval_data$Open * (1 + 0.05), both_first = both_first)) / scale_fct 
...
model_low_binary <- fit_binary_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "Low_0", 10)
save_model_hdf5(model_low_binary, "data/model_low_binary.hdf5")

model_high_binary <- fit_binary_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "High_0", 10)
save_model_hdf5(model_low_binary, "data/model_high_binary.hdf5")

model_close_binary <- fit_binary_model(data_wide_3_all, data_wide_3, train_idx, test_idx, "Close_0", 10)
save_model_hdf5(model_low_binary, "data/model_close_binary.hdf5")
