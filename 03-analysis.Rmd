...
```{r, keras-combined-individual-softmax, echo=FALSE}
...
# Low_0
low_model <- keras::keras_model_sequential() %>%
...
low_model %>% keras::compile(
  optimizer = 'adam', 
  loss = 'sparse_categorical_crossentropy',
  metrics = c('accuracy')
)

low_history <- low_model %>% keras::fit(
  train_data,
  train_labels,
  epochs = 50,
  batch_size = 128, 
...
# High_0
high_model <- keras::keras_model_sequential() %>%
  keras::layer_dense(units = 512, activation = "relu",  input_shape = dim(train_data)[2]) %>%
  keras::layer_dense(units = 512, activation = "relu") %>%
  keras::layer_dense(units = n_groups_per_col^length(cols), activation = "softmax")

high_model %>% keras::compile(
  optimizer = 'adam', 
  loss = 'sparse_categorical_crossentropy',
  metrics = c('accuracy')
)

high_history <- high_model %>% keras::fit(
  train_data,
  train_labels,
  epochs = 50,
  batch_size = 128, 
...
# Close_0
close_model <- keras::keras_model_sequential() %>%
  keras::layer_dense(units = 512, activation = "relu",  input_shape = dim(train_data)[2]) %>%
  keras::layer_dense(units = 512, activation = "relu") %>%
  keras::layer_dense(units = n_groups_per_col^length(cols), activation = "softmax")

close_model %>% keras::compile(
  optimizer = 'adam', 
  loss = 'sparse_categorical_crossentropy',
  metrics = c('accuracy')
)

close_history <- close_model %>% keras::fit(
  train_data,
  train_labels,
  epochs = 50,
  batch_size = 128, 
