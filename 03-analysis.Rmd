...
pred_classes <- predict_classes(model_low, test_data, batch_size = 1024)
pred_prob <- predict_proba(model_low, test_data, batch_size = 1024)


get_class_prices <- function(discretization) {
  class_borders <- discretization$borders %>% set_names(c("Bucket", "Lower_Border", "Upper_Border"))
