...
fit_model_cat <- function(x, y, epochs = 10, callbacks = NULL, n_groups = 30) {
  model <- keras_model_sequential() 
  model %>% 
    layer_dense(units = 512, activation = 'relu', input_shape = ncol(data)) %>% 
    layer_dense(units = 512, activation = 'relu') %>%
    layer_dense(units = n_groups, activation = 'softmax')
...
pred_dep_path  <- "data/models/dep/categorical/2_dense_512/pred.rds"
if (file.exists(model_dep_path) && file.exists(history_dep_path) && file.exists(pred_dep_path)) {
  model_dep <- load_model_hdf5(model_dep_path)
  history_dep <- readRDS(history_dep_path)
  pred_dep <- readRDS(pred_dep_path)
} else {
  ...
  pred_dep <- predict_proba(model_dep$model, x[test_idx, ], batch_size = 512)
  saveRDS(pred_dep, pred_dep_path)
}
