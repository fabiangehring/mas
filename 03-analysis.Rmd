...
quotes_clean_hash <- "94b2b8bdc8775dcce36c1b48bc79cbe4"
...
quotes_clean <- arrow::read_feather(paste0("tmp/quotes_clean_", quotes_clean_hash, ".feather"))
data <- quotes_clean %>%
...
# arrow::write_feather(data_all, paste0("tmp/data_all_", quotes_clean_hash, ".feather"))
data_all <- arrow::read_arrow(paste0("tmp/data_all_", quotes_clean_hash, ".feather"))

# remove unneeded (after normalization) day before window start
data <- dplyr::select(data_all, c("Ticker", "Date", str_subset(names(data_all), paste0("_[^", window + 1, "]$"))))

# drop NA (for window and current)
data <- tidyr::drop_na(data)

# remove current date except "Open"
data <- dplyr::select(data, c("Ticker", "Date", "Open_0", str_subset(names(data), paste0("_[^", 0, "]$"))))

# sort columns nicely
data <- data %>% dplyr::select(c("Ticker", "Date"), sort(stringr::str_subset(names(.), "_[0-9]+$"), decreasing = TRUE))
id_cols <- c("Ticker", "Date")
data_all <- data[, id_cols] %>% inner_join(data_all, by = id_cols)
rm(quotes_clean)
...
data <- arrange(data, Date, Ticker)
knn_eucl <- find_nn_chunkwise(
  data = data,
  distance = "euclidean",
  k = 75,
  n_chunks = 200,
...
saveRDS(knn_eucl, "tmp/knn_eucl.rds")
