...
Rcpp::sourceCpp('src/find_best_buy_sell.cpp')
...
ind_categorical$discretization <- list(
  low = multivariate_discretization(data_wide_3_all, train_idx, test_idx, "Low_0", n_groups_per_col),
  high = multivariate_discretization(data_wide_3_all, train_idx, test_idx, "High_0", n_groups_per_col),
  close = multivariate_discretization(data_wide_3_all, train_idx, test_idx, "Close_0", n_groups_per_col)
)

### model for close
model_names <- c("low_pred_prob", "high_pred_prob", "close_pred_prob")
model_paths <- paste0("data/models/ind/categorical/", ind_categorical_model_names, ".feather")

if (all(map_lgl(ind_categorical_model_paths, file.exists))) {
  ind_categorical$pred$low = read_feather(model_paths[1])
  ind_categorical$pred$high = read_feather(model_paths[2])
  ind_categorical$pred$close = read_feather(model_paths[3])
  
} else {
  stop(paste0("At least one ind_categorical model does not exist. Please execute python code first."))
  
  ind_categorical_labels <- tibble(
    low = ind_categorical$discretization$low %$% groups,
    high = ind_categorical$discretization$high %$% groups, 
    close = ind_categorical$discretization$close %$% groups
  )
...
hist_data_low <- eval_hist_data(
  borders = ind_categorical$discretization$low$borders, 
  prob = unlist(ind_categorical$pred$low[curr_eval_id, ], use.names = FALSE), 
  group = "Tief"
)

hist_data_high <- eval_hist_data(
  borders = ind_categorical$discretization$high$borders, 
  prob = unlist(ind_categorical$pred$high[curr_eval_id, ], use.names = FALSE), 
  group = "Hoch"
)

hist_data_close <- eval_hist_data(
  borders = ind_categorical$discretization$close$borders, 
  prob = unlist(ind_categorical$pred$close[curr_eval_id, ], use.names = FALSE), 
  group = "Schluss"
)
...
plot_price_histogram(
  ...
  title = "Histogramm prognostizierter Tiefst-, Höchst- und Schlusspreise"
)
```

```{r nn-indipendent-categorical-evaluation}

price_scenarios <- eval_price_scenarios(ind_categorical$discretization)

eval_buy_sell_scenarios <- function() {
  # find all buy sell scenarios
buy_sell_combinations <- expand(mid_prices, Low, High) %>% select(buy = Low, sell = High)
implausible_buy_sell_combinations_idx <- which((buy_sell_combinations$buy > buy_sell_combinations$sell) |
                                                 !is.finite(buy_sell_combinations$buy) | 
                                                 !is.finite(buy_sell_combinations$sell))
plausible_buy_sell_combinations_idx <- setdiff(seq_len(nrow(buy_sell_combinations)), implausible_buy_sell_combinations_idx)
...
find_optimal_buy_sell <- function(low_pred_prob, high_pred_prob, close_pred_prob, buy_first_payoffs, sell_first_payoffs, mc.cores = getOption("mc.cores", 2L)) {
  
  find_best_buy_sell_parallel <- function(probs, payoffs) {
    find_best_buy_sell(
      low_prob = probs$low_prob,
      high_prob = probs$high_prob,
      close_prob = probs$close_prob,
  ...
  low_prob <- as.matrix(low_pred_prob) %>% unname() %>%.[seq_len(sample_n),, drop = FALSE]
  high_prob <- as.matrix(high_pred_prob) %>% unname() %>% .[seq_len(sample_n),, drop = FALSE]
  close_prob <- as.matrix(close_pred_prob) %>% unname() %>% .[seq_len(sample_n),, drop = FALSE]
  buy_first_payoffs <- as.matrix(buy_first_payoffs) %>% unname()
  sell_first_payoffs <- as.matrix(buy_first_payoffs) %>% unname()
  
  split_probs <- map(chunk2(seq_len(sample_n), mc.cores), ~list(low_prob = low_prob[., ], high_prob = high_prob[., ], close_prob = close_prob[., ]))
  pbmclapply(
    X = split_probs,
...
bench <- buy_sell_combinations[out, ]

test_sample <- data_wide_3_all[test_idx, ] %>% select(Close_1, Low = Low_0, High = High_0, Close_0) %>% .[seq_len(sample_n), ]
sum(calc_payoff_const_gamma(test_sample))
sum(calc_payoff_const_gamma(test_sample, buy = bench$buy, sell = bench$sell))
