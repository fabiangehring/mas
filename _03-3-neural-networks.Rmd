...
mid_prices <- eval_mid_prices(ind_categorical$discretization)
...
sample_n <- length(test_idx)
REMOVE
sample_n <- 100

set.seed(123456)
both_first <- c("buy", "sell")[sample(c(1, 2), length(test_idx), replace = TRUE)]

optimal_buy_sell_idx <- find_optimal_buy_sell(
  low_pred_prob = ind_categorical$pred$low[seq_len(sample_n), ],
  high_pred_prob = ind_categorical$pred$high[seq_len(sample_n), ],
  close_pred_prob = ind_categorical$pred$close[seq_len(sample_n), ],
  buy_first_payoffs = buy_first_payoffs,
  sell_first_payoffs = sell_first_payoffs,
  both_first = both_first[seq_len(sample_n)],
  mc.cores = 3
)

bench <- buy_sell_scenarios[optimal_buy_sell_idx, ]
