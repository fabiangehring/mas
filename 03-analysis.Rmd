...
data_all <- quotes_clean %>%
  ...
  normalize_quotes(paste0("Close_", window + 1), setdiff(names(data), c("Ticker", "Date")))
...
id_cols <- c("Ticker", "Date")
curr_id <- data[idx, id_cols]

data_wide_curr <- data_all[idx, ]
data_wide_nn <- data_all %>% inner_join(select(data, id_cols),  by = id_cols)

plot_nn(
  data_wide_curr = data_all[idx, ],
  data_wide_nn = data_all[as.integer(knn_eucl_idx[idx, ]), ]
)
