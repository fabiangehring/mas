...
mid_prices <- bind_rows(
  mutate(ind_categorical_discretization_low$borders, type = "Low"),
  mutate(ind_categorical_discretization_high$borders, type = "High"),
  mutate(ind_categorical_discretization_close$borders, type = "Close")
...
implausible_price_combinations_idx <- which(
...
plausible_price_combinations_idx <- setdiff(seq_len(nrow(price_combinations)), implausible_price_combinations_idx)
price_combinations[implausible_price_combinations_idx, ] <- 100
...
low_prob <- as.matrix(ind_categorical_models$low_pred_prob) %>% unname() %>%.[seq_len(sample_n), ]
high_prob <- as.matrix(ind_categorical_models$high_pred_prob) %>% unname() %>% .[seq_len(sample_n), ]
close_prob <- as.matrix(ind_categorical_models$close_pred_prob) %>% unname() %>% .[seq_len(sample_n), ]
payoffs <- as.matrix(buy_first_payoffs) %>% unname()

split_probs <- map(chunk2(seq_len(sample_n), n_cores), ~list(low_prob = low_prob[., ], high_prob = high_prob[., ], close_prob = close_prob[., ]))
out <- pbmclapply(X = split_probs, FUN = find_best_buy_sell_parallel, payoffs = payoffs, mc.cores = n_cores) %>% unlist()
...
all_prob <- expand(tibble(low_prob = low_prob, high_prob = high_prob, close_prob = close_prob), low_prob, high_prob, close_prob) %>%
  mutate(prob = low_prob * high_prob * close_prob)
