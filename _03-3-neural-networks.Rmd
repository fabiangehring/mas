...
```{r nn-indipendent-categorical, echo=FALSE}

### model for close
n_groups_per_col <- 30
ind_categorical_model_names <- c("indipendent_categorical_low", "indipendent_categorical_high", "indipendent_categorical_close")
ind_categorical_model_paths <- paste0("data/", ind_categorical_model_names, ".hdf5")

if (all(map_lgl(ind_categorical_model_paths, file.exists))) {
  ind_categorical_models <- map(ind_categorical_model_paths, load_model_hdf5) %>% setNames(ind_categorical_model_names)
} else {
  stop(paste0("At least one indipendent_categorical model does not exist. Please execute python code first."))
  
  ind_categorical_labels_low <- multivariate_discretization(data_wide_3_all, train_idx, test_idx, "Low_0", n_groups_per_col) %$% groups
  ind_categorical_labels_high <- multivariate_discretization(data_wide_3_all, train_idx, test_idx, "High_0", n_groups_per_col) %$% groups
  ind_categorical_labels_close <- multivariate_discretization(data_wide_3_all, train_idx, test_idx, "Close_0", n_groups_per_col) %$% groups
...
n_groups_per_col <- 30
ind_binary_model_names <- c("indipendent_binary_low", "indipendent_binary_high", "indipendent_binary_close")
ind_binary_model_paths <- paste0("data/", ind_binary_model_names, ".hdf5")

if (all(map_lgl(ind_binary_model_paths, file.exists))) {
  ind_binary_models <- map(ind_binary_model_paths, load_model_hdf5) %>% setNames(ind_binary_model_names)
} else {
  stop(paste0("At least one indipendent_binary model does not exist. Please execute python code first."))
