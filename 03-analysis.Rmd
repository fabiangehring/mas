...
# ### Testing only
# set.seed(951753)
# fract_idx <- sample(seq_len(nrow(data_wide_3)), 100000)
# 
# data_wide_3 <- data_wide_3[fract_idx, ]
# data_wide_3_all <- data_wide_3_all[fract_idx, ]
# ###
...
breaks_low <- find_breaks(data_wide_3_all$Low_0, train_idx, test_idx, n_group)

breaks_high <- find_breaks(data_wide_3_all$High_0, train_idx, test_idx, n_group)
groups_high <- as.integer(cut(data_wide_3_all$High_0, breaks = breaks_high), include.lowest = TRUE)

breaks_close <- find_breaks(data_wide_3_all$Close_0, train_idx, test_idx, n_group)
groups_close <- as.integer(cut(data_wide_3_all$Close_0, breaks = breaks_close), include.lowest = TRUE)

groups_map <- expand.grid(groups_low = seq_len(n_group), groups_high = seq_len(n_group), groups_close = seq_len(n_group)) %>% mutate(group = seq_len(nrow(.)) - 1)

all_labels <- tibble(groups_low = groups_low, groups_high = groups_high, groups_close = groups_close) %>% 
  left_join(groups_map, by = c("groups_low", "groups_high", "groups_close")) %>%
  group_by(group) %>%
  summarize(n = n())
...
model <- keras_model_sequential() %>%
  layer_dense(units = 256, activation = "relu",  input_shape = dim(train_data)[2]) %>%
  layer_dense(units = 256, activation = "relu") %>%
  layer_dense(units = n_group^3, activation = "softmax")
...
quantile_generator <- function(input_vector, quantiles = 4) { 
  tiles <- quantile(input_vector, probs = seq(0, 1, 1/quantiles))
  temp <- rep(NA,length(input_vector)) 
  for (i in 2:length(tiles)){ 
    temp[input_vector <= tiles[i] & is.na(temp)] <- i 
  }
  return(temp) 
} 
tempvec <- runif(100,0,1)
df <- data.frame(v1= tempvec, v2= quantile_generator(tempvec, 4)) split(df, df$v2) 
