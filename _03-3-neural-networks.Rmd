...
low_prob <- as.matrix(ind_categorical_models$low_pred_prob) %>% unname() %>%.[seq_len(sample_n),, drop = FALSE]
high_prob <- as.matrix(ind_categorical_models$high_pred_prob) %>% unname() %>% .[seq_len(sample_n),, drop = FALSE]
close_prob <- as.matrix(ind_categorical_models$close_pred_prob) %>% unname() %>% .[seq_len(sample_n),, drop = FALSE]
...
out <- pbmclapply(X = split_probs, FUN = find_best_buy_sell_parallel, payoffs = payoffs, mc.cores = n_cores) %>% unlist(use.names = FALSE)
