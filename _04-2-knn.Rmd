...
if (!all(map_lgl(nn_3_eucl_path, ~file.exists(.)))) {
  ...
  nn_3_eucl <- find_nn(data = data_knn_3, distance = "euclidean", k = 50, mc.cores = ceiling(detectCores() * 0.55))
...
if (!all(map_lgl(nn_3_manh_path, ~file.exists(.)))) {
  ...
  nn_3_manh <- find_nn(data = data_knn_3, distance = "manhattan", k = 50, mc.cores = ceiling(detectCores() * 0.55))
