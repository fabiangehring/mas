...
payoff_factor_test_eucl <- calc_nn_payoff_factor(
...
k <- 20
data_test <- select(data_knn_3_incl_0, Date, Close_1, Open_0, Low_0, High_0, Close_0)[test_idx, ]
pred_test <- pred_nn(select(data_test, Low_0, High_0), as.matrix(nn_3_manh_idx[test_idx, seq_len(k)]))

plot_ratio_history(data_test %>% rename(Low = "Low_0", High = "High_0"), data_pred = pred_test %>% rename(Buy = Low_0, Sell = High_0), both_first = both_first[test_idx])
```
