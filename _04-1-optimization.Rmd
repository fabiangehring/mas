...
if (file.exists(variation_factor_open_file_name)) {
  ...
  data <- data_wide_3_all %>% select(Close_1, Open = Open_0, Low = Low_0, High = High_0, Close_0)
...
#   rm(quotes_clean)
  ...
  scale_fct_train <- sum(calc_payoff_const_gamma(data[train_idx, ], both_first = both_first[train_idx]))
  opt_payoff_sym(0.0375, data[train_idx, ], "Open", both_first[train_idx], scale_fct_train)
